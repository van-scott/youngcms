...
//jbsxBox右侧列表刷新 新增后调用
function jbsxBoxDialogDone(json) {
    DWZ.ajaxDone(json);
    var $pagerForm = $("#pagerForm", navTab.getCurrentPanel());
    var args = $pagerForm.size() > 0 ? $pagerForm.serializeArray() : {};
    var url=$pagerForm.attr("action");
    $("#jbsxBox",navTab.getCurrentPanel()).ajaxUrl({
        type: "POST",
        url: url,
        data:args,
        callback: function() {
        	$.pdialog.closeCurrent();
        	$("#jbsxBox",navTab.getCurrentPanel()).find("[layoutH]").layoutH()
        }
    })
}
//jbsxBox右侧列表刷新 删除后调用
